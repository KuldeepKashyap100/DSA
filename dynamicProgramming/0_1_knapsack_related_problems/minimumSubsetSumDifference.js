 ...
 * 
 * Range of possible sums -> [0, sigma(arr[i])] (0 because we can consider null set and max sum by adding all elements).
 * assume s1 + s2 = sigma(arr[i]) (where s1 is sum of elements in s1 and s2 is sum of elements in s2).
 * then s2 = sigma(arr[i]) - s1;
 * 
 * we have to find abs(s2 - s1) = minDifference
